<script>
  ...
  // Page-enter gate so intros only run client-side
  let mounted = false;
  onMount(() => requestAnimationFrame(() => (mounted = true)));

  // Global speed knobs (bigger = slower)
  const isReduced =
    typeof matchMedia !== "undefined" &&
    matchMedia("(prefers-reduced-motion: reduce)").matches;

  const DUR_MULT = 5;  // durations
  const DELAY_MULT = 3; // staggers
  const T = (ms) => (isReduced ? 0 : Math.round(ms * DUR_MULT));
  const D = (ms) => (isReduced ? 0 : Math.round(ms * DELAY_MULT));

  // Small scatter helpers for staggered list items
  ...
  // Primary: derived from $page.params (works in normal SvelteKit nav)
  $: currentSeriesSlug = $page.params?.series ?? "";

  // Fallback: force-update from the URL on mount and after navigation
  const deriveSlugFrom = (url) => {
    try {
      const parts = url.pathname.split("/");
      // /catalog/series/[series] => ["", "catalog", "series", "<slug>", ...]
      return parts[3] || "";
    } catch {
      return "";
    }
...
>
  ...
  <!-- Mobile drawer (animate in/out from left) -->
  {#if sidebarOpen}
    <aside
  ...
  <!-- Desktop fixed sidebar (fade/scale on page mount) -->
{#if mounted}
  <div
...
{/if}
  ...
  <main class="flex-1 p-6 overflow-y-auto md:pt-6">
    ...
    <!-- Slot content wrapper to animate page content -->
    {#if mounted}
